...
// This is the component for the navigation links, now reusable
    ...
    <nav className="grid items-start px-2 text-sm font-medium lg:px-4">
      {navItems.map((item) => (
        <Link
          key={item.label}
          href={item.href}
          onClick={onLinkClick} // Close the sheet when a link is clicked
          className={cn(
      ...
      <div className="flex flex-col">
        ...
        <header className="flex h-14 items-center gap-4 border-b bg-muted/40 px-4 md:hidden">
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
           ...
           {/* We can add the user profile dropdown here for mobile as well */}
        </header>
